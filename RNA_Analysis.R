...
rna.raw <- as.matrix(read.delim("rna_raw.txt", row.names = 1))
...
ha <- rowAnnotation(mark_genes = anno_mark(at = c(9,20,63,75,77,84,86,88,96),labels = rownames(heat.sig)[c(9,20,63,75,77,84,86,88,96)]))
...
hall.comp.all.deg <- compareCluster(lst.deg, fun="enricher", TERM2GENE=msig_hall)
...
hm.hall <- cbind(data.frame(t(hm.hall[,1:60])), "overgroup" = rna.meta$overgroup)
