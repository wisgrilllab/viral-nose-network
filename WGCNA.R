...
rna.term <- c("T1_INF_M","T1_INF_S","T2_INF_M","T2_INF_S","T3_INF_M","T3_INF_S","T4_INF_M","T4_INF_S","T5_INF_M","T5_INF_S",
...
data.rna.term <- data.rna.term[!(rownames(data.rna.term) %in% 'T1_INF_S'), ]
trait.term <- trait.term[!(rownames(trait.term) %in% 'T1_INF_S'), ]
rna.meta.term <- rna.meta.term[!(rownames(rna.meta.term) %in% 'T1_INF_S'), ]
...
interesting.modules <- c("purple", "brown", "yellow", "salmon", "greenyellow", "magenta", "pink")
AM.indices <- c("AM1", "AM2", "AM3", "AM4", "AM5", "AM6", "AM7")
...
MEs.adult.sum$subgroup = c(rep("MOCK",6), rep("IAV/RSV", 3), rep("RSV", 12))
...
color.heat.adult = list(
  Module = c(purple="purple", brown="brown", yellow="yellow", salmon="salmon", greenyellow="greenyellow", magenta="magenta", pink="pink")
)
...
adult.enrich.vec = c("R-HSA-156902","R-HSA-9010553","R-HSA-71291",
                     "R-HSA-913531", "R-HSA-1169410", "R-HSA-1834949",
                     "R-HSA-6798695", "R-HSA-195721", "R-HSA-9013694",
                     "R-HSA-611105", "R-HSA-1428517", "R-HSA-72172")
...
# Calculate Network
net.term <- blockwiseModules(data.rna.term, power = 14, networkType = "signed hybrid", maxBlockSize = 30000,
                             corType = "pearson", TOMType = "signed", minModuleSize = 50, deepSplit = 2,
                             mergeCutHeight=0.25, numericLabels = TRUE, pamRespectsDendro = T, verbose=3)
...
# Select modules of interest which show correleation in the heatmap
interesting.modules <- c("red", "magenta", "black", "royalblue", "turquoise", "yellow", "blue", "salmon")
TM.indices <- c("TM1", "TM2", "TM3", "TM4", "TM5", "TM6", "TM7", "TM8")
...
MEs.term.sum <- eigengenes.term[1] %>% as.data.frame() %>% 
                ...
                filter(modules %in% interesting.modules) %>% 
                arrange(modules) %>% as.data.frame()
MEs.term.sum$subgroup = c(rep("MOCK",6), rep("IAV", 9), rep("RSV", 9))
...
heat.term.final <- heat.term[,-20] %>% t() %>% as.data.frame() %>%
                   mutate(group = rna.meta.term$condition) %>%
                   group_by(group) %>% summarise_all(mean) %>% t() %>% as.data.frame()
...
color.heat.term = list(
  Module = c(red="red", magenta="magenta", black="black", royalblue="royalblue", turquoise="turquoise", yellow="yellow", blue="blue", salmon="salmon")
)
...
term.enrich.vec = c("R-HSA-196849","R-HSA-196854",
                    "R-HSA-3299685", "R-HSA-70635",
                    "R-HSA-351202","R-HSA-5358346",
                    "R-HSA-191273","R-HSA-8957322",
                    "R-HSA-192823", "R-HSA-168255",
                    "R-HSA-1989781", "R-HSA-400206",
                    "R-HSA-913531", "R-HSA-168898",
                    "R-HSA-2470946", "R-HSA-68884")
...
# Select modules of interest which show correleation in the heatmap
interesting.modules <- c("tan","red","darkred","salmon","black","greenyellow","brown","pink","cyan","lightcyan")
PM.indices <- c("PM1", "PM2", "PM3", "PM4", "PM5", "PM6", "PM7", "PM8","PM9","PM10")
...
MEs.preterm.sum <- eigengenes.preterm[1] %>% as.data.frame() %>% 
                   ...
                   filter(modules %in% interesting.modules) %>% 
                   arrange(modules) %>% as.data.frame()
MEs.preterm.sum$subgroup = c(rep("MOCK",18), rep("IAV", 9), rep("RSV", 3))
...
color.heat.preterm = list(
  Module = c(tan="tan",red="red",darkred="darkred",salmon="salmon",black="black",greenyellow="greenyellow",brown="brown",pink="pink",cyan="cyan",lightcyan="lightcyan")
)
...
write.table(rea.enrich.preterm@compareClusterResult, "rea_enrich_wgcna_term.txt", quote = F, sep="\t")

preterm.enrich.vec = c("R-HSA-3299685","R-HSA-72737",
                       "R-HSA-77289",
                       "R-HSA-5358508",
                       "R-HSA-1855167",
                       "R-HSA-913531", "R-HSA-168898",
                       "R-HSA-5676590", "R-HSA-169911",
                       "R-HSA-191273", "R-HSA-71336")
...
net = blockwiseConsensusModules(multiExpr, power = c(9,14,14), maxBlockSize = 30000,
...
cons.interesting.module <- c("darkred",
                             "blue","salmon","royalblue",
                             "magenta","darkturquoise") 
...
rea.cons.filt <- rea.cons.filt %>% filter(ID %in% rea.vect) 
...
kt=SoftConnectivity(data.rna.term, 14)
...
write.table(diff.con.adult2, "diff.con.adult2.txt", quote = FALSE, sep="\t")
